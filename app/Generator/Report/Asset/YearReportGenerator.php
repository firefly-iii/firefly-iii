 ...
 * 
 * Generates asset allocation and net worth reports for a year period.
 * Shows asset growth, monthly net worth trends, and investment performance.
 */
class YearReportGenerator implements ReportGeneratorInterface
{
    ...
    /**
     * Generates the asset report for a year period.
     *
    ...
    public function generate(): string
    {
        ...
        try {
            return view('reports.asset.year', compact('accountIds', 'reportType'))
                ->with('start', $this->start)
                ->with('end', $this->end)
                ->with('accounts', $this->accounts)
                ->render();
        } catch (Throwable $e) {
            Log::error(sprintf('Cannot render reports.asset.year: %s', $e->getMessage()));
