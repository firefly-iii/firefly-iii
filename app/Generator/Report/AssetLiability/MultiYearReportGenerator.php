 ...
 * 
 * Generates a comprehensive Asset & Liability report for multiple years.
 * Shows historical account balance trends and long-term changes.
 */
class MultiYearReportGenerator implements ReportGeneratorInterface
{
    ...
    /**
     * Generates the Asset & Liability report for multiple years.
     *
    ...
    public function generate(): string
    {
        ...
        try {
            return view('reports.assetliability.multi-year', compact('accountIds', 'reportType'))
                ->with('start', $this->start)
                ->with('end', $this->end)
                ->with('accounts', $this->accounts)
                ->render();
        } catch (Throwable $e) {
            Log::error(sprintf('Cannot render reports.assetliability.multi-year: %s', $e->getMessage()));
