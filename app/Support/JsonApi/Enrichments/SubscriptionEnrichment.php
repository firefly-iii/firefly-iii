...
class SubscriptionEnrichment implements EnrichmentInterface
{
    ...
    {
        $notes = Note::query()
            ->whereIn('noteable_id', $this->subscriptionIds)
            ->whereNotNull('notes.text')
            ->where('notes.text', '!=', '')
            ->where('noteable_type', Bill::class)
    ...
    private function getLastPaidDate(array $paidData): null|Carbon
    {
    ...
    private function getNextExpectedMatch(array $payDates): null|Carbon
    {
        // next expected match
        $nem          = null;
        ...
        if (null !== $firstPayDate) {
            ...
            if ($this->start instanceof Carbon && $nemDate->lt($this->start) || $this->end instanceof Carbon && $nemDate->gt($this->end)) {
                $nem          = null;
                $nemDate      = null;
                $firstPayDate = null;
            }
    ...
    private function getNextExpectedMatchDiff(null|Carbon $nem, array $payDates): string
    {
