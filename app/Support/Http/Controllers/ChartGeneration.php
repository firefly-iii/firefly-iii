...
trait ChartGeneration
{
     ...
     * @SuppressWarnings(PHPMD.ExcessiveMethodLength)
     */
    protected function makePeriodChartByCostCenter(CostCenter $costCenter, Carbon $start, Carbon $end): array // chart helper method.
    {
        $cache = new CacheProperties;
        $cache->addProperty($start);
        $cache->addProperty($end);
        ...
        while ($start <= $end) {
            $spent                           = $repository->spentInPeriod(new Collection([$costCenter]), $accounts, $start, $start);
            $earned                          = $repository->earnedInPeriod(new Collection([$costCenter]), $accounts, $start, $start);
            $sum                             = bcadd($spent, $earned);
            $label                           = trim(app('navigation')->periodShow($start, $step));
            $chartData[0]['entries'][$label] = round(bcmul($spent, '-1'), 12);
            $chartData[1]['entries'][$label] = round($earned, 12);
