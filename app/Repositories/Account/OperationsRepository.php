...
class OperationsRepository implements OperationsRepositoryInterface
{
    ...
    /**
     * TODO same as sumIncome and sumExpense, but copied
     *
     * @inheritDoc
     */
    ...
    public function sumExpensesBySource(
        ...
        /** @var GroupCollectorInterface $collector */
        $collector = app(GroupCollectorInterface::class);
        $collector->setUser($this->user)->setRange($start, $end)->setTypes([TransactionType::WITHDRAWAL])->withAccountInformation();
        ...
        if (null !== $currency) {
            /** @var GroupCollectorInterface $collector */
            $collector = app(GroupCollectorInterface::class);
            $collector->setUser($this->user)->setRange($start, $end)->setTypes([TransactionType::WITHDRAWAL])->setForeignCurrency($currency)
        ...
        foreach ($journals as $journal) {
            $key                = sprintf('%s-%s', $journal['source_account_id'], $journal['currency_id']);
            $array[$key]        = $array[$key] ?? [
                    'id'                      => $journal['source_account_id'],
                    'name'                    => $journal['source_account_name'],
                    'iban'                    => $journal['source_account_iban'],
                    'sum'                     => '0',
            ...
            $array[$key]['sum'] = bcadd($array[$key]['sum'], app('steam')->negative($journal['amount']));

            // also do foreign amount:
            if (0 !== (int)$journal['foreign_currency_id']) {
                $key                = sprintf('%s-%s', $journal['source_account_id'], $journal['foreign_currency_id']);
                $array[$key]        = $array[$key] ?? [
                        'id'                      => $journal['source_account_id'],
                        'name'                    => $journal['source_account_name'],
                        'iban'                    => $journal['source_account_iban'],
                ...
                $array[$key]['sum'] = bcadd($array[$key]['sum'], app('steam')->negative($journal['foreign_amount']));
            }
        }
    ...
    public function sumExpensesByDestination(
        ...
        foreach ($journals as $journal) {
            $key                = sprintf('%s-%s', $journal['destination_account_id'], $journal['currency_id']);
            $array[$key]        = $array[$key] ?? [
                    'id'                      => $journal['destination_account_id'],
                    'name'                    => $journal['destination_account_name'],
                    'iban'                    => $journal['destination_account_iban'],
                    'sum'                     => '0',
                    'currency_id'             => $journal['currency_id'],
                    'currency_name'           => $journal['currency_name'],
                    'currency_symbol'         => $journal['currency_symbol'],
            ...
            // also do foreign amount:
            if (0 !== (int)$journal['foreign_currency_id']) {
                $key                = sprintf('%s-%s', $journal['destination_account_id'], $journal['foreign_currency_id']);
                $array[$key]        = $array[$key] ?? [
                        'id'                      => $journal['destination_account_id'],
                        'name'                    => $journal['destination_account_name'],
                        'iban'                    => $journal['destination_account_iban'],
    ...
    public function sumIncomeByDestination(
        ...
        /** @var GroupCollectorInterface $collector */
        $collector = app(GroupCollectorInterface::class);
        $collector->setUser($this->user)->setRange($start, $end)->setTypes([TransactionType::DEPOSIT])->withAccountInformation();
        ...
        if (null !== $currency) {
            ...
            $collector = app(GroupCollectorInterface::class);
            $collector->setUser($this->user)->setRange($start, $end)->setTypes([TransactionType::DEPOSIT])
                      ->setForeignCurrency($currency)->withAccountInformation();
        ...
        foreach ($journals as $journal) {
            ...
            $array[$key]['sum'] = bcadd($array[$key]['sum'], app('steam')->positive($journal['amount']));
            ...
            if (0 !== (int)$journal['foreign_currency_id']) {
                ...
                $array[$key]['sum'] = bcadd($array[$key]['sum'], app('steam')->positive($journal['foreign_amount']));
            }
        }
