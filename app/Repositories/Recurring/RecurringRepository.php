...
class RecurringRepository implements RecurringRepositoryInterface, UserGroupInterface
{
    ...
    public function createdPreviously(Recurrence $recurrence, Carbon $date): bool
    {
        ...
        foreach ($set as $journalMeta) {
            $count = TransactionJournalMeta::query()->where(static function (Builder $q2) use ($date): void {
                $string = (string) $date;
    ...
    public function getAll(): Collection
    {
        ...
        return Recurrence::query()->with(['TransactionCurrency', 'TransactionType', 'RecurrenceRepetitions', 'RecurrenceTransactions'])
            ->orderBy('active', 'DESC')
            ->orderBy('title', 'ASC')
            ->get()
        ;
