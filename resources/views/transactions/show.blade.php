                        ...
                        <ul class="dropdown-menu">
                            ...
                            {% if groupArray.transactions[0].type != 'opening balance' and groupArray.transactions[0].type != 'reconciliation' %}
                                ...
                                <li><a href="#" class="clone-transaction" data-id="{{ transactionGroup.id }}"><span
                                            class="bi bi-copy"></span> {{ 'clone'|_ }}</a></li>
                                <li><a href="#" class="clone-transaction-and-edit" data-id="{{ transactionGroup.id }}"><span
                                            class="bi bi-copy"></span> {{ 'clone_and_edit'|_ }}</a></li>
                            @endif
                            @endif
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                    <table class="table table-hover">
                        ...
                        @endif
                        <tr>
                            <td class="thirty">{{ 'total_amount'|_ }}</td>
                            <td>
                                {% for amount in amounts %}
                                    {% if first.transaction_type_type == 'Withdrawal' %}
                                        {% if amount.approximate %}~ @endif{{ format_amount_by_symbol(amount.amount,amount.symbol, amount.decimal_places) }}{% if loop.index0 != amounts|length -1 %}, @endif
                                    {% elseif first.transaction_type_type == 'Deposit' %}
                                        {% if amount.approximate %}~ @endif{{ format_amount_by_symbol(amount.amount*-1,amount.symbol, amount.decimal_places) }}{% if loop.index0 != amounts|length -1 %}, @endif
                                    {% elseif first.transaction_type_type == 'Transfer' %}
                                        <span class="text-info money-transfer">
                                        {% if amount.approximate %}~ @endif{{ format_amount_by_symbol(amount.amount*-1, amount.symbol, amount.decimal_places, false) }}{% if loop.index0 != amounts|length -1 %}, @endif
                                    </span>
        ...
        {% for index, journal in selectedGroup.transactions %}
            <div class="col-lg-{{ boxSize }}">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a href="#" class="link-modal"
                                       data-journal="{{ journal.transaction_journal_id }}"><span
                                            class="fa fa-link"></span>{{ 'link_transaction'|_ }}</a></li>
                                <li role="separator" class="divider"></li>
                                <li>
                                    <a href="{{ route('rules.create-from-journal', [journal.transaction_journal_id]) }}"><span
                                            class="fa fa-random"></span>{{ 'create_rule_from_transaction'|_ }}
                                    </a></li>
                                <li>
                                    <a href="{{ route('recurring.create-from-journal', [journal.transaction_journal_id]) }}"><span
                                            class="fa fa-paint-brush"></span>{{ 'create_recurring_from_transaction'|_ }}
                                    </a></li>
                                @endif
                            </ul>
                        </div>
                ...
                {% if attachments[journal.transaction_journal_id]|length > 0 %}
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ 'attachments'|_ }}</h3>
                        </div>
                        <div class="card-body p-0">
                ...
                {% if events[journal.transaction_journal_id]|length > 0 %}
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ 'piggy_events'|_ }}</h3>
                        </div>
                        <div class="card-body p-0">
                            <table class="table table-hover">
                                {% for event in events[journal.transaction_journal_id] %}
                                    <tr>
                                        <td class="thirty">{{ event.amount|raw }}</td>
                                        <td>
