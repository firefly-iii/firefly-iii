<table class="table table-valign-middle table-sm table-hover sortable table-sm">
    <thead>
    <tr>
    ...
    @foreach($report as $info)
        <tr>
            <td data-value="{{ $info['title'] }}">
                @if($info['id'] !== 0)
                    <a class="btn btn-outline-secondary btn-sm" href="{{ route('categories.show', [$info['id']]) }}"><span class="bi bi-box-arrow-up-right"></span></a>
                @else
                    <a class="btn btn-outline-secondary btn-sm" href="{{ route('categories.no-category') }}"><span class="bi bi-box-arrow-up-right"></span></a>
                @endif
            </td>
            <td data-value="{{ $info['title'] }}">
