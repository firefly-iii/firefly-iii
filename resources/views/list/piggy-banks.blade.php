<table class="table table-hover table-sm" id="piggy-sortable">
    <thead>
    <tr>
        <th colspan="2">&nbsp;</th>
        <th>{{ 'piggy_bank'|_ }}</th>
    ...
    {% for objectGroupOrder, objectGroup in piggyBanks %}
        {% if objectGroup.piggy_banks|length > 0 %}
            <tbody class="piggy-connected-list" {% if objectGroupOrder != 0 %}data-title="{{ objectGroup.object_group_title }}"@elsedata-title=""@endif>
            <tr>
                <td colspan="2">&nbsp;</td>
                ...
                <tr class="piggy-sortable" data-id="{{ piggy.id }}" data-name="{{ piggy.name }}" data-order="{{ piggy.order }}">
                    ...
                    <td class="visible-xs visible-sm hidden-md hidden-lg">
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('piggy-banks.remove-money-mobile', piggy.id) }}" class="btn btn-outline-secondary btn-xs"><span class="fa fa-minus"></span></a>
                            <a href="{{ route('piggy-banks.add-money-mobile', piggy.id) }}" class="btn btn-outline-secondary btn-xs"><span class="bi bi-plus-circle"></span></a>
                        </div>
                    </td>
                    <td  class="hidden-sm hidden-xs sixty-px">
                        <span class="fa fa-bars piggy-handle"></span>
                        <span class="loadSpin"></span>
                    </td>
                    <td class="hidden-sm hidden-xs hundred-px">
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('piggy-banks.edit', piggy.id) }}" class="btn btn-outline-secondary btn-xs"><span class="bi bi-pencil"></span></a>
                            <a href="{{ route('piggy-banks.delete', piggy.id) }}" class="btn btn-danger btn-xs"><span class="bi bi-trash"></span></a>
                        </div>
                    </td>
                    <td>
                    ...
                    <td class="hidden-sm hidden-xs forty-px">
                        ...
                        {% if piggy.left_to_save > 0 or null == piggy.left_to_save %}
                            <a href="{{ route('piggy-banks.add-money', piggy.id) }}" class="btn btn-outline-secondary btn-xs addMoney" data-id="{{ piggy.id }}">
                    ...
                    <td class="hidden-sm hidden-xs text-right">
                        {% if null != piggy.target_amount and 0 != piggy.target_amount %}
                            ...
                            {% if convertToPrimary and piggy.currency_id != $primaryCurrency->id and null != piggy.pc_target_amount %}
                                (<span title="{{ 'target_amount'|_ }}">{{ format_amount_by_symbol(piggy.pc_target_amount,$primaryCurrency->symbol, $primaryCurrency->decimal_places) }}</span>)
                            @endif
                        @endif
                    </td>
                    <td class="hidden-sm hidden-xs text-right">
                        {% if piggy.left_to_save > 0 %}
                            ...
                            {% if convertToPrimary and piggy.currency_id != $primaryCurrency->id and null != piggy.pc_left_to_save %}
                                (<span title="{{ 'left_to_save'|_ }}">{{ format_amount_by_symbol(piggy.pc_left_to_save, $primaryCurrency->symbol,$primaryCurrency->decimal_places) }}</span>)
                            @endif
                        @endif
                    </td>
                    <td class="hidden-sm hidden-xs text-right">
