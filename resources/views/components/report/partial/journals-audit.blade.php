    ...
    <tr data-date="{{ $journal['date']->format('Y-m-d') }}" data-id="{{ $journal['transaction_journal_id'] }}">
        ...
        <td class="hide-buttons">
            <div class="btn-group btn-group-sm">
                <a href="{{ route('transactions.edit',$journal['transaction_group_id']) }}?_from={{ urlencode($FF3_FROM) }}" class="btn btn-sm btn-outline-secondary"><span class="bi bi-pencil"></span></a>
                <a href="{{ route('transactions.delete',$journal['transaction_group_id']) }}?_from={{ urlencode($FF3_FROM) }}" class="btn btn-sm btn-danger"><span class="bi bi-trash"></span></a>
            </div>
        </td>
